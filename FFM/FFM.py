...
class FM(object):
    ...
    def generator(self, data, batch_size):
        row = len(data[0])
        num_batch = np.ceil(row / self.batch_size)
        for i in range(num_batch):
            end = min(i + self.batch_size, row)
            yield data[0][i:end], data[1][i:end]

    def traindata(self, data, batch_size=128):
        if isinstance(data, Generator):
            self.train_generator = data
        else:
            self.train_generator = self.generator(data, batch_size)

    def validdata(self, data, batch_size=128):
        if isinstance(data, Generator):
            self.valid_generator = data
        else:
            self.valid_generator = self.generator(data, batch_size)
